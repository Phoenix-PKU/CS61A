...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        second = Pair('lambda', Pair(formals, body))
        return do_define_form(Pair(name, Pair(second, nil)), env)
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    formal = bindings
    while formal != nil:
        validate_form(formal.first, 2, 2)
        names = Pair(formal.first.first, names)
        vals = Pair(scheme_eval(formal.first.rest.first, env), vals)
        formal = formal.rest
